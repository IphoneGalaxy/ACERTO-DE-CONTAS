...
const BASE = 'https://iphonegalaxy.github.io/ACERTO-DE-CONTAS/';
...
(async () => {
  ...
  try {
    ...
    log('02-welcome', hasWelcome, hasWelcome ? 'Tela inicial genérica visível' : 'Tela de boas-vindas não encontrada');
    if (hasWelcome) await shot(page, '01-welcome');
    ...
    if (hasWelcome) {
      ...
      if (namesVisible) await shot(page, '02-names');
      ...
      if (cardsStep) await shot(page, '03-cards');
      ...
      if (codeStep) await shot(page, '04-household-code');
      ...
      if (hasSelect) await shot(page, '05-user-select');
      ...
      if (dashboard) await shot(page, '06-dashboard-ana');

      const hasAnaLabel = bodyText.includes('Ana') || (await page.locator('body').innerText()).includes('Ana');
      ...
      if (cardOption) await shot(page, '07-modal-novo');
      ...
      if (ajusteB) await shot(page, '08-dashboard-bruno-ajuste');
    }
  } catch (err) {
